...
class MockLLMProvider(LLMProvider):
    ...
    async def generate_structured(
        ...
        if issubclass(response_schema, InterpretationResponse):
            ...
            # If batched prompt with "--- Question "
            if "--- Question " in prompt:
                sections = prompt.split("--- Question ")
                verdicts: list[EvidenceVerdict] = []
                for sec in sections[1:]:
                    q_lines = [line.replace("Question:", "").strip() for line in sec.splitlines() if line.startswith("Question:")]
                    q_text = q_lines[0] if q_lines else "General inquiry"
                    ...
                    elif "checkout" in q_lower or "database connection" in q_lower or "error rates" in q_lower or "pool timeout" in q_lower or "deployment" in q_lower or "independent" in q_lower:
                        verdicts.append(
                            EvidenceVerdict(
                                question=q_text,
                                evidence_ids_cited=sec_uuids[:2],
